...
def tail_points(opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    return 2 * abs(rightmost(1, opponent_score) - rightmost(2, opponent_score)) + 1
    # END PROBLEM 2
...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    ans, indx = 0, 0
    for i in range(1, 10):
        tmp = make_averaged(roll_dice, total_samples)
        tmp = tmp(i,dice)
        if ans < int(tmp):
            indx = i
            ans = tmp
...
def tail_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    tail_score = tail_points(opponent_score)  # Remove this line once implemented.
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    zero_roll = score + tail_points(opponent_score)
    if perfect_square(zero_roll) or not tail_strategy(score, opponent_score, threshold, num_rolls):
        return 0
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    """
    # BEGIN PROBLEM 12
    roll_6 = make_averaged(roll_dice, total_samples=100)
    roll_0 = make_averaged(roll_dice, total_samples=100)
    
    for i in range(1, 3):
        if roll_0(i) > roll_6(i):
            return 0
